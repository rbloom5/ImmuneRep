...
def run_format_MITCR(ids, meta_file = 'metadata.txt'):
	...
	# try:
		...
		# print bash('java -jar ./software/vdjtools-1.0-SNAPSHOT.jar Convert -S mitcr %s formatted'%ids)
		# copy_to_s3('formatted.%s'%ids, '.', 't-cell-rep/mitcr-output')
	...
	# except:
	# 	return 'error running MITCR on %s'%ids
...
# for ids in MS_files:
	# copy_from_s3(ids, '.', 't-cell-rep/mitcr-output')
	# run_format_MITCR(ids, meta_file = 'zehnder_metadata.txt')
