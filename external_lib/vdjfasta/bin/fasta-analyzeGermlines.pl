#!/usr/bin/perl -w

# Author:  Jacob Glanville 
# Contact: jacob <dot> glanville <at> pfizer <dot> com
#
# Glanville J, Zhai W, Berka J et al. Precise determination of the diversity 
# of a combinatorial antibody library gives insight into the human immunoglobulin 
# repertoire. Proc Natl Acad Sci USA. 2009;106:20216–20221

use Getopt::Long;
use vars qw($rootdir);
BEGIN{
  use File::Basename;
  use Cwd 'abs_path';
  $rootdir = dirname(dirname(abs_path($0)));
};
use lib "$rootdir/lib";
use VDJFasta;
use strict;

################################ Arguments ################################

my ($seqfile,$domain,$minlength,$min_shm,$max_shm,$isRedundant)=GatherOptions();
 
################################ Inputs    ################################

my $fasta=VDJFasta->new();
   $fasta->loadSeqs($seqfile);

################################ Analysis  ################################

   if($fasta->parseHeaderAnnotations()){
     $fasta->reportGermFreqs($domain,$minlength,$min_shm,$max_shm,$isRedundant);
   }

############################### Subroutines ###############################

sub GatherOptions {
  my $file   =   ""; # sequence file
  my $domain =   "";
  my $minlen =   "";
  my $minshm =   "";
  my $maxshm =   "";
  my $redund =    0;

  GetOptions(
    "--file=s"  => \$file,
    "--domain=s"  => \$domain,
    "--minlen=s"  => \$minlen,
    "--minshm=s"  => \$minshm,
    "--maxshm=s"  => \$maxshm,
    "--redund=s" => \$redund
  );
  unless(-f $file ){
    print "\nUsage: $0\n";
    print " --file     dna sequence file in fasta format\n";
    exit;
  }
  return ($file,$domain,$minlen,$minshm,$maxshm,$redund);
}

