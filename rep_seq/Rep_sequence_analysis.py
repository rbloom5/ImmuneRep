...
class Rep_seq:
	...
	def __init__(self, filepath, num = False, ABtype = []): #filepath must be file with extension XXX.VDJ.H3.L3.CH1.fa from VDJfasta
		...
		self.features['V_freqs'] = {}
		self.features['V_fractions'] = {}
		for germ in self.Reads_split_by_VJ:
			self.features['V_freqs'][germ] = len(self.Reads_split_by_VJ[germ])
			self.features['V_fractions'][germ] = len(self.Reads_split_by_VJ[germ])/float(self.features['num_Reads'])
	...
	def plot_V_fractions(self, f4=False):
		if f4:
			...
			data = self.features['V_fractions']
